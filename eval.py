...
def evaluation_metrics(pred_dir,gt_dir):
    ...
    for i in range(len(pred_pth_list)):
        print(f'------------{i}---------------')
        pred = cv2.imread(os.path.join(pred_dir,pred_pth_list[i]), 0)[500:1000,1500:3500]
        _, pred = cv2.threshold(pred, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        mask = cv2.imread(os.path.join(gt_dir,gt_pth_list[i]), 0)[500:1000,1500:3500]
...
# evaluation_metrics(r'E:\Dataset\temp\unet',r'E:\Dataset\temp\rst')
